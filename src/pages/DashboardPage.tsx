            ...
            return (
              <>
                <div className="kpi-val" style={{ color: 'var(--t5)' }}>
                  {refPrice && cash > 0 ? fmtU(cash / refPrice, 0) + ' USDT' : cash > 0 ? fmtQ(cash) : t('setCash')}
                </div>
                <div className="kpi-sub">@ {refPrice ? fmtP(refPrice) : '3.7'} QAR</div>
              </>
            );
          })()}
        </div>
      ...
      <div className="kpis" style={{ marginTop: 0 }}>
        {(() => {
          ...
          return (
            <div className="kpi-card" style={{ cursor: 'pointer', position: 'relative' }} onClick={() => setExpandedNewKpi(isExpanded ? null : 'roi')}>
              <div className="kpi-head">
                <span className="kpi-badge" style={{ color: 'var(--good)', borderColor: 'color-mix(in srgb,var(--good) 30%,transparent)', background: 'color-mix(in srgb,var(--good) 10%,transparent)' }}>💹</span>
              </div>
              ...
              <div style={{ display: 'flex', gap: 4, marginTop: 4, alignItems: 'center' }}>
                ...
                <span className="kpi-badge" style={{ fontSize: 9, padding: '1px 6px', color: 'var(--muted)' }}>Orders</span>
                <span style={{ fontSize: 9, color: 'var(--warn)', marginLeft: 'auto' }}>💡 {isExpanded ? 'collapse' : 'tap to expand'}</span>
              </div>
              {isExpanded && (
                <div style={{ marginTop: 8, padding: '8px 10px', borderTop: '1px solid var(--line)', fontSize: 11, color: 'var(--t3)' }}>
                  Return on invested capital for {roiPeriod} — normalizes profit vs full book.
                </div>
              )}
            </div>
          );
        ...
        <div className="kpi-card" style={{ cursor: 'pointer' }} onClick={() => setExpandedNewKpi(expandedNewKpi === 'cycle' ? null : 'cycle')}>
          ...
          <div style={{ display: 'flex', gap: 4, marginTop: 4 }}>
            <span className="kpi-badge" style={{ fontSize: 9, padding: '1px 6px' }}>FIFO</span>
            <span className="kpi-badge" style={{ fontSize: 9, padding: '1px 6px', color: 'var(--muted)' }}>Stock</span>
            <span style={{ fontSize: 9, color: 'var(--warn)', marginLeft: 'auto' }}>💡 {expandedNewKpi === 'cycle' ? 'collapse' : 'tap to expand'}</span>
          </div>
        </div>
        {merchantDealKpis && (
          <>
            <div className="kpi-card">
              ...
              <div className={`kpi-val ${merchantDealKpis.outNet >= 0 ? 'good' : 'bad'}`}>{merchantDealKpis.outNet >= 0 ? '+' : ''}{fmtQWithUnit(merchantDealKpis.outNet)}</div>
              <div className="kpi-sub">My cut: {fmtQRaw(merchantDealKpis.dealDetails.filter(d => d.direction === 'outgoing').reduce((s, d) => s + d.myShare, 0))} QAR</div>
            </div>
            <div className="kpi-card">
              ...
              <div className={`kpi-val ${merchantDealKpis.inNet >= 0 ? 'good' : 'bad'}`}>{merchantDealKpis.inNet >= 0 ? '+' : ''}{fmtQWithUnit(merchantDealKpis.inNet)}</div>
              <div className="kpi-sub">My cut: {fmtQRaw(merchantDealKpis.dealDetails.filter(d => d.direction === 'incoming').reduce((s, d) => s + d.myShare, 0))} QAR</div>
            </div>
          </>
        )}
      </div>
          ...
          <div className="panel-body" style={{ height: 180 }}>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={trendData}>
                <defs>
                  <linearGradient id="gRevenue" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#6366f1" stopOpacity={0.3} /><stop offset="95%" stopColor="#6366f1" stopOpacity={0} /></linearGradient>
                ...
                <Area type="monotone" dataKey="revenue" name={t('volume')} stroke="#6366f1" fill="url(#gRevenue)" strokeWidth={1.5} dot={false} />
                <Area type="monotone" dataKey="profit" name={t('netProfitLabel')} stroke="#22c55e" fill="url(#gProfit)" strokeWidth={2} dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
