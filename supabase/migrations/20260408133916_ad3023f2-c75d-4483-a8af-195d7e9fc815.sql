...
-- 1. Enum types
DO $$ BEGIN CREATE TYPE public.chat_room_type AS ENUM ('merchant_private', 'merchant_client', 'merchant_collab'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE public.chat_member_role AS ENUM ('owner', 'admin', 'member', 'guest'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE public.chat_message_type AS ENUM ('text', 'voice_note', 'image', 'file', 'system', 'call_summary', 'order_card', 'payment_card', 'reaction_burst'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE public.chat_call_status AS ENUM ('ringing', 'active', 'ended', 'missed', 'declined', 'failed', 'no_answer'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE public.chat_encryption_mode AS ENUM ('none', 'tls_only', 'server_e2ee', 'client_e2ee'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- 2. chat_room_policies
...
INSERT INTO public.chat_room_policies (room_type, encryption_mode, retention_hours, allow_calls, moderation_level, history_searchable, watermark_enabled, screenshot_protection, max_file_size_mb) VALUES
...
CREATE TABLE IF NOT EXISTS public.chat_rooms (
  ...
  last_message_id UUID, last_message_at TIMESTAMPTZ, last_message_preview TEXT,
  is_direct BOOLEAN NOT NULL DEFAULT FALSE,
  is_announcement_only BOOLEAN NOT NULL DEFAULT FALSE,
  metadata JSONB NOT NULL DEFAULT '{}',
  migrated_from TEXT, migrated_source_id UUID,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

UPDATE public.chat_rooms cr SET policy_id = (SELECT id FROM public.chat_room_policies WHERE room_type = cr.type) WHERE policy_id IS NULL;
...
CREATE TABLE IF NOT EXISTS public.chat_room_members (
  ...
  is_muted BOOLEAN NOT NULL DEFAULT FALSE, muted_until TIMESTAMPTZ,
...
CREATE TABLE IF NOT EXISTS public.chat_messages (
  ...
  content TEXT NOT NULL DEFAULT '', metadata JSONB NOT NULL DEFAULT '{}',
  reply_to_id UUID REFERENCES public.chat_messages(id) ON DELETE SET NULL,
  forwarded_from_id UUID REFERENCES public.chat_messages(id) ON DELETE SET NULL,
  client_nonce TEXT UNIQUE,
  is_edited BOOLEAN NOT NULL DEFAULT FALSE, edited_at TIMESTAMPTZ,
  ...
  expires_at TIMESTAMPTZ, view_once BOOLEAN NOT NULL DEFAULT FALSE,
...
CREATE TABLE IF NOT EXISTS public.chat_attachments (
  ...
  storage_path TEXT NOT NULL, cdn_url TEXT,
  file_name TEXT NOT NULL, file_size BIGINT NOT NULL, mime_type TEXT NOT NULL,
  thumbnail_path TEXT, duration_ms INTEGER, width INTEGER, height INTEGER,
  waveform JSONB, checksum_sha256 TEXT,
  is_validated BOOLEAN NOT NULL DEFAULT FALSE,
  is_encrypted BOOLEAN NOT NULL DEFAULT FALSE, iv TEXT, auth_tag TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.chat_calls (
  ...
  end_reason TEXT, ice_config JSONB, quality_stats JSONB,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.chat_device_keys (
  ...
  public_key TEXT NOT NULL, key_id INTEGER, signature TEXT,
...
DO $$ DECLARE _t TEXT;
BEGIN
  FOREACH _t IN ARRAY ARRAY['chat_rooms','chat_room_members','chat_messages','chat_message_receipts','chat_message_reactions','chat_attachments','chat_calls','chat_call_participants','chat_presence','chat_typing_state'] LOOP
    BEGIN EXECUTE format('ALTER PUBLICATION supabase_realtime ADD TABLE public.%I', _t); EXCEPTION WHEN duplicate_object THEN NULL; END;
  END LOOP;
END $$;
...
RETURNS public.chat_member_role LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
...
CREATE POLICY "receipts_self_upsert" ON public.chat_message_receipts FOR ALL TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
...
CREATE POLICY "reactions_self_all" ON public.chat_message_reactions FOR ALL TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid() AND public.fn_is_chat_member(room_id, auth.uid()));
...
CREATE POLICY "calls_member_select" ON public.chat_calls FOR SELECT TO authenticated USING (public.fn_is_chat_member(room_id, auth.uid()) AND EXISTS (SELECT 1 FROM public.chat_rooms r JOIN public.chat_room_policies p ON p.id = r.policy_id WHERE r.id = room_id AND p.allow_calls = TRUE));
DROP POLICY IF EXISTS "calls_member_insert" ON public.chat_calls;
CREATE POLICY "calls_member_insert" ON public.chat_calls FOR INSERT TO authenticated WITH CHECK (initiated_by = auth.uid() AND public.fn_is_chat_member(room_id, auth.uid()) AND EXISTS (SELECT 1 FROM public.chat_rooms r JOIN public.chat_room_policies p ON p.id = r.policy_id WHERE r.id = room_id AND p.allow_calls = TRUE));
...
CREATE POLICY "call_participants_select" ON public.chat_call_participants FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM public.chat_calls c WHERE c.id = call_id AND public.fn_is_chat_member(c.room_id, auth.uid())));
DROP POLICY IF EXISTS "call_participants_upsert" ON public.chat_call_participants;
CREATE POLICY "call_participants_upsert" ON public.chat_call_participants FOR ALL TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "presence_member_select" ON public.chat_presence;
CREATE POLICY "presence_member_select" ON public.chat_presence FOR SELECT TO authenticated USING (TRUE);
DROP POLICY IF EXISTS "presence_self_upsert" ON public.chat_presence;
CREATE POLICY "presence_self_upsert" ON public.chat_presence FOR ALL TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
...
CREATE POLICY "typing_self_upsert" ON public.chat_typing_state FOR ALL TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid() AND public.fn_is_chat_member(room_id, auth.uid()));
...
CREATE POLICY "device_keys_self_write" ON public.chat_device_keys FOR ALL TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
...
CREATE POLICY "e2ee_sessions_sender_write" ON public.chat_e2ee_sessions FOR ALL TO authenticated USING (sender_id = auth.uid()) WITH CHECK (sender_id = auth.uid());

DROP POLICY IF EXISTS "audit_admin_select" ON public.chat_audit_events;
CREATE POLICY "audit_admin_select" ON public.chat_audit_events FOR SELECT TO authenticated USING (public.has_role(auth.uid(), 'admin'));
DROP POLICY IF EXISTS "audit_system_insert" ON public.chat_audit_events;
CREATE POLICY "audit_system_insert" ON public.chat_audit_events FOR INSERT TO authenticated WITH CHECK (TRUE);

-- 20. RPC Functions

CREATE OR REPLACE FUNCTION public.chat_get_or_create_direct_room(_other_user_id UUID, _room_name TEXT DEFAULT NULL)
...
CREATE OR REPLACE FUNCTION public.chat_create_merchant_client_room(_customer_user_id UUID, _room_name TEXT DEFAULT NULL)
RETURNS UUID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE _me UUID := auth.uid(); _room_id UUID; _ua UUID; _ub UUID; _policy UUID;
BEGIN
  IF _me IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
...
CREATE OR REPLACE FUNCTION public.chat_send_message(_room_id UUID, _content TEXT, _type TEXT DEFAULT 'text', _metadata JSONB DEFAULT '{}', _reply_to_id UUID DEFAULT NULL, _client_nonce TEXT DEFAULT NULL, _expires_at TIMESTAMPTZ DEFAULT NULL, _view_once BOOLEAN DEFAULT FALSE, _watermark_text TEXT DEFAULT NULL)
...
BEGIN
  ...
  INSERT INTO public.chat_messages (room_id, sender_id, type, content, metadata, reply_to_id, client_nonce, expires_at, view_once, watermark_text)
  VALUES (_room_id, _me, _type::public.chat_message_type, _content, _metadata, _reply_to_id, _client_nonce, _expires_at, _view_once, _watermark_text) RETURNING * INTO _msg;
  UPDATE public.chat_rooms SET last_message_id = _msg.id, last_message_at = _msg.created_at, last_message_preview = left(_content, 120), updated_at = now() WHERE id = _room_id;
  INSERT INTO public.chat_message_receipts (message_id, room_id, user_id, status) VALUES (_msg.id, _room_id, _me, 'read') ON CONFLICT (message_id, user_id) DO NOTHING;
  RETURN NEXT _msg;
END; $$;
...
BEGIN
  ...
  SELECT m.id, _room_id, _me, 'read', now() FROM public.chat_messages m
  ...
  ON CONFLICT (message_id, user_id) DO UPDATE SET status = 'read', updated_at = now() WHERE chat_message_receipts.status <> 'read';
  UPDATE public.chat_room_members SET last_read_message_id = COALESCE(_up_to_message_id, (SELECT id FROM public.chat_messages WHERE room_id = _room_id AND is_deleted = FALSE ORDER BY created_at DESC LIMIT 1)), last_read_at = now() WHERE room_id = _room_id AND user_id = _me;
END; $$;
...
BEGIN
  IF NOT public.fn_is_chat_member(_room_id, _me) THEN RAISE EXCEPTION 'Not a member'; END IF;
  INSERT INTO public.chat_typing_state (room_id, user_id, is_typing, expires_at, updated_at) VALUES (_room_id, _me, _is_typing, now() + interval '8 seconds', now())
  ON CONFLICT (room_id, user_id) DO UPDATE SET is_typing = EXCLUDED.is_typing, expires_at = EXCLUDED.expires_at, updated_at = now();
END; $$;

CREATE OR REPLACE FUNCTION public.chat_set_presence(_status TEXT DEFAULT 'online', _device_info JSONB DEFAULT '{}')
RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE _me UUID := auth.uid();
BEGIN
  INSERT INTO public.chat_presence (user_id, status, last_seen_at, device_info, updated_at) VALUES (_me, _status, now(), _device_info, now())
  ON CONFLICT (user_id) DO UPDATE SET status = EXCLUDED.status, last_seen_at = now(), device_info = EXCLUDED.device_info, updated_at = now();
END; $$;
...
CREATE OR REPLACE FUNCTION public.chat_initiate_call(_room_id UUID)
RETURNS UUID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE _me UUID := auth.uid(); _call_id UUID; _policy RECORD;
BEGIN
  ...
  INSERT INTO public.chat_calls (room_id, initiated_by, status) VALUES (_room_id, _me, 'ringing') RETURNING id INTO _call_id;
  INSERT INTO public.chat_call_participants (call_id, user_id, status, joined_at) VALUES (_call_id, _me, 'connected', now());
  INSERT INTO public.chat_call_participants (call_id, user_id, status) SELECT _call_id, m.user_id, 'ringing' FROM public.chat_room_members m WHERE m.room_id = _room_id AND m.user_id <> _me AND m.removed_at IS NULL;
  PERFORM public.chat_send_message(_room_id, '📞 Call started', 'system', jsonb_build_object('call_id', _call_id, 'event', 'call_initiated'), NULL, gen_random_uuid()::text);
  RETURN _call_id;
END; $$;
...
CREATE OR REPLACE FUNCTION public.chat_end_call(_call_id UUID, _end_reason TEXT DEFAULT 'ended')
RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE _me UUID := auth.uid(); _room_id UUID; _dur INTEGER;
BEGIN
  SELECT room_id, EXTRACT(EPOCH FROM (now() - connected_at))::INTEGER INTO _room_id, _dur FROM public.chat_calls WHERE id = _call_id;
  UPDATE public.chat_calls SET status = CASE WHEN _end_reason = 'declined' THEN 'declined'::public.chat_call_status WHEN _end_reason = 'missed' THEN 'missed'::public.chat_call_status ELSE 'ended'::public.chat_call_status END, ended_at = now(), duration_seconds = GREATEST(_dur, 0), end_reason = _end_reason WHERE id = _call_id;
  UPDATE public.chat_call_participants SET status = 'disconnected', left_at = now() WHERE call_id = _call_id AND user_id = _me;
  IF _end_reason NOT IN ('declined', 'missed') THEN
    PERFORM public.chat_send_message(_room_id, '📞 Call ended · ' || COALESCE(_dur::text || 's', '0s'), 'call_summary', jsonb_build_object('call_id', _call_id, 'duration_seconds', _dur), NULL, gen_random_uuid()::text);
  END IF;
END; $$;

CREATE OR REPLACE FUNCTION public.chat_push_ice_candidate(_call_id UUID, _candidate JSONB)
RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN UPDATE public.chat_call_participants SET ice_candidates = ice_candidates || _candidate WHERE call_id = _call_id AND user_id = auth.uid(); END; $$;

CREATE OR REPLACE FUNCTION public.chat_get_or_create_collab_room(_name TEXT DEFAULT 'Merchants Hub')
RETURNS UUID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE _me UUID := auth.uid(); _room_id UUID; _policy UUID;
BEGIN
  SELECT r.id INTO _room_id FROM public.chat_rooms r JOIN public.chat_room_members m ON m.room_id = r.id AND m.user_id = _me AND m.removed_at IS NULL WHERE r.type = 'merchant_collab' LIMIT 1;
  IF _room_id IS NOT NULL THEN RETURN _room_id; END IF;
  SELECT r.id INTO _room_id FROM public.chat_rooms r WHERE r.type = 'merchant_collab' AND r.is_announcement_only = FALSE ORDER BY r.created_at ASC LIMIT 1;
  IF _room_id IS NOT NULL THEN
    INSERT INTO public.chat_room_members (room_id, user_id, role) VALUES (_room_id, _me, 'member') ON CONFLICT (room_id, user_id) DO UPDATE SET removed_at = NULL, role = 'member';
    RETURN _room_id;
  END IF;
  SELECT id INTO _policy FROM public.chat_room_policies WHERE room_type = 'merchant_collab';
  INSERT INTO public.chat_rooms (type, name, created_by, policy_id, is_direct) VALUES ('merchant_collab', _name, _me, _policy, FALSE) RETURNING id INTO _room_id;
  INSERT INTO public.chat_room_members (room_id, user_id, role) VALUES (_room_id, _me, 'owner');
  RETURN _room_id;
END; $$;

CREATE OR REPLACE FUNCTION public.chat_search_messages(_room_id UUID, _query TEXT, _limit INT DEFAULT 40)
RETURNS SETOF public.chat_messages LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
...
DO $$ DECLARE _t TEXT;
BEGIN
  FOREACH _t IN ARRAY ARRAY['chat_rooms', 'chat_room_members', 'chat_messages', 'chat_room_policies', 'chat_presence', 'chat_typing_state'] LOOP
    EXECUTE format('DROP TRIGGER IF EXISTS trg_%s_updated_at ON public.%s; CREATE TRIGGER trg_%s_updated_at BEFORE UPDATE ON public.%s FOR EACH ROW EXECUTE FUNCTION public.fn_set_updated_at()', _t, _t, _t, _t);
  END LOOP;
END; $$;

-- 24. DATA MIGRATION (adapted to actual os_rooms/os_messages schema)

-- 24.1 os_rooms → chat_rooms (no last_message_at column in os_rooms, compute from os_messages)
INSERT INTO public.chat_rooms
  (id, type, name, created_by, is_direct, last_message_at, migrated_from, migrated_source_id, created_at, updated_at)
SELECT
  r.id, 'merchant_private'::public.chat_room_type, r.name,
  (SELECT mp.user_id FROM public.os_room_members m JOIN public.merchant_profiles mp ON mp.merchant_id = m.merchant_id
   WHERE m.room_id = r.id ORDER BY CASE WHEN m.role = 'owner' THEN 0 ELSE 1 END, m.joined_at ASC LIMIT 1),
  TRUE,
  (SELECT MAX(om.created_at) FROM public.os_messages om WHERE om.room_id = r.id),
  'os_rooms', r.id, r.created_at, r.updated_at
FROM public.os_rooms r
ON CONFLICT (id) DO NOTHING;

UPDATE public.chat_rooms cr SET policy_id = (SELECT id FROM public.chat_room_policies WHERE room_type = cr.type) WHERE policy_id IS NULL AND migrated_from = 'os_rooms';

-- 24.2 os_room_members → chat_room_members
INSERT INTO public.chat_room_members (room_id, user_id, role, joined_at)
SELECT m.room_id, mp.user_id,
  CASE m.role WHEN 'owner' THEN 'owner'::public.chat_member_role WHEN 'admin' THEN 'admin'::public.chat_member_role ELSE 'member'::public.chat_member_role END,
  m.joined_at
FROM public.os_room_members m JOIN public.merchant_profiles mp ON mp.merchant_id = m.merchant_id
WHERE EXISTS (SELECT 1 FROM public.chat_rooms WHERE id = m.room_id)
ON CONFLICT (room_id, user_id) DO NOTHING;

-- 24.3 os_messages → chat_messages (os_messages has: sender_merchant_id, content, expires_at, created_at, no body_json/message_type/reply_to_message_id/client_nonce/sender_id)
INSERT INTO public.chat_messages
  (id, room_id, sender_id, type, content, expires_at, created_at, updated_at)
SELECT
  m.id, m.room_id,
  COALESCE(mp.user_id, '00000000-0000-0000-0000-000000000000'::UUID),
  'text'::public.chat_message_type,
  COALESCE(m.content, ''),
  m.expires_at, m.created_at, m.created_at
...
-- 24.4 os_messages read state → receipts
INSERT INTO public.chat_message_receipts (message_id, room_id, user_id, status, updated_at)
SELECT m.id, m.room_id, mp.user_id,
  CASE WHEN m.read_at IS NOT NULL THEN 'read' ELSE 'delivered' END,
  COALESCE(m.read_at, m.created_at)
FROM public.os_messages m
JOIN public.merchant_profiles mp ON mp.merchant_id = m.sender_merchant_id
WHERE EXISTS (SELECT 1 FROM public.chat_rooms WHERE id = m.room_id)
  AND mp.user_id IS NOT NULL AND NOT m.is_deleted
ON CONFLICT (message_id, user_id) DO NOTHING;

-- 24.5 message_reactions → chat_message_reactions
INSERT INTO public.chat_message_reactions (id, message_id, room_id, user_id, emoji, created_at)
SELECT r.id, r.message_id, cm.room_id, mp.user_id, r.reaction, r.created_at
FROM public.message_reactions r
JOIN public.chat_messages cm ON cm.id = r.message_id
JOIN public.merchant_profiles mp ON mp.merchant_id = r.user_id
WHERE mp.user_id IS NOT NULL
ON CONFLICT (message_id, user_id, emoji) DO NOTHING;

-- 24.6 customer_merchant_connections → chat_rooms + chat_messages
DO $$
DECLARE _conn RECORD; _room_id UUID; _policy UUID; _merchant_uid UUID;
BEGIN
  SELECT id INTO _policy FROM public.chat_room_policies WHERE room_type = 'merchant_client';
  FOR _conn IN SELECT * FROM public.customer_merchant_connections WHERE status <> 'blocked' LOOP
    SELECT user_id INTO _merchant_uid FROM public.merchant_profiles WHERE merchant_id = _conn.merchant_id LIMIT 1;
    IF _merchant_uid IS NULL THEN CONTINUE; END IF;
    INSERT INTO public.chat_rooms (type, created_by, policy_id, is_direct, migrated_from, migrated_source_id, created_at)
    VALUES ('merchant_client', _merchant_uid, _policy, TRUE, 'customer_chat', _conn.id, _conn.created_at)
    ON CONFLICT DO NOTHING RETURNING id INTO _room_id;
    IF _room_id IS NULL THEN SELECT id INTO _room_id FROM public.chat_rooms WHERE migrated_source_id = _conn.id AND migrated_from = 'customer_chat'; END IF;
    IF _room_id IS NULL THEN CONTINUE; END IF;
    INSERT INTO public.chat_room_members (room_id, user_id, role, joined_at) VALUES (_room_id, _merchant_uid, 'owner', _conn.created_at), (_room_id, _conn.customer_user_id, 'member', _conn.created_at) ON CONFLICT (room_id, user_id) DO NOTHING;
    INSERT INTO public.chat_messages (room_id, sender_id, type, content, created_at, updated_at)
    SELECT _room_id, CASE WHEN cm.sender_role = 'merchant' THEN _merchant_uid ELSE cm.sender_user_id END, 'text', COALESCE(cm.content, ''), cm.created_at, cm.created_at
    FROM public.customer_messages cm WHERE cm.connection_id = _conn.id ON CONFLICT DO NOTHING;
  END LOOP;
END; $$;
...
VALUES ('chat-attachments', 'chat-attachments', FALSE, 104857600,
  ARRAY['image/jpeg','image/png','image/gif','image/webp','image/heic','video/mp4','video/webm','audio/mpeg','audio/ogg','audio/wav','audio/webm','audio/mp4','application/pdf','application/msword','application/vnd.openxmlformats-officedocument.wordprocessingml.document','application/vnd.ms-excel','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet','text/plain','text/csv'])
ON CONFLICT (id) DO NOTHING;

DROP POLICY IF EXISTS "chat_attachments_read" ON storage.objects;
CREATE POLICY "chat_attachments_read" ON storage.objects FOR SELECT TO authenticated USING (bucket_id = 'chat-attachments' AND EXISTS (SELECT 1 FROM public.chat_attachments a WHERE a.storage_path = name AND public.fn_is_chat_member(a.room_id, auth.uid())));
DROP POLICY IF EXISTS "chat_attachments_upload" ON storage.objects;
CREATE POLICY "chat_attachments_upload" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'chat-attachments');
DROP POLICY IF EXISTS "chat_attachments_delete" ON storage.objects;
CREATE POLICY "chat_attachments_delete" ON storage.objects FOR DELETE TO authenticated USING (bucket_id = 'chat-attachments' AND (storage.foldername(name))[1] = auth.uid()::text);
